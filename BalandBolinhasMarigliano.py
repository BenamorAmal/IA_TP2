...
def printConstants():
	'''
	fonction de debug pour afficher les constantes
	'''
	print("#####################")
	print("")
	print("N: %s" % N)
	print("MUTATION_PROB: %s" % MUTATION_PROB)
	print("SWAP_PROB: %s" % SWAP_PROB)
	print("CROSSED_N: %s" % CROSSED_N)
	print("RANDOM_N: %s" % RANDOM_N)
	print("DECAL_N: %s" % DECAL_N)
...
def generateRandomPath(cities):
	"""
	:param cities:
	:return: Un chemin du genre: "1-5-6-4" en tant qu'objet Solution
	"""
...
def selection(population):
	...
	for solution in population:
		# Corrige une erreur de suppression de résultats lorsque la moyenne est fixée
...
def selectionRanking(population):
	...
	# Pas besoin de copier car utilisation seulement dans cette fonctions
